...
const STATUS = {
  Running:   { color: "bg-[#635bff]", pulse: true, label: "Running" },
  Waiting:   { color: "bg-[#f5a623]", pulse: false, label: "Waiting" },
  Completed: { color: "bg-[#0caf60]", pulse: false, label: "Done" },
  Failed:    { color: "bg-[#df1b41]", pulse: false, label: "Failed" },
  created:   { color: "bg-[#635bff]", pulse: true, label: "Starting" },
} as const;
  ...
  if (tasks.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-6">
        ...
        <p className="text-[13px] text-[#697386] text-center leading-5">
          No tasks yet.<br />Describe what you need and hit Run Agent.
        </p>
      </div>
    );
  }
      ...
      {tasks.map((task) => {
        ...
        return (
          <button
            ...
            className={`w-full text-left px-4 py-3 border-b border-[#e3e8ee] transition-colors ${
              activeTask === task.id ? "bg-[#f0efff]" : "hover:bg-[#f6f9fc]"
            }`}
          >
